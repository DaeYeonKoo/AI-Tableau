...
<style>
  ...
  *{box-sizing:border-box;}
  ...
  .stack .card{flex:1;display:flex;flex-direction:column;}
  .stack .card > div:last-child{flex:1;}
...
__WORLDMAP__
        ...
        <div class="stack">
          ...
          <div class="card">
            <h2>Top 5 생산공장 <span class="card-sub">(클레임 금액 기준)</span></h2>
            <div id="rankPlant"></div>
          </div>
        </div>
      </div>
    </section>
...
function rebuildPage1(rowsNoRange, rowsInRange){
  ...
  const plantSums = {}; rowsInRange.forEach(r=>{ plantSums[r[1]] = (plantSums[r[1]]||0)+r[4]; });
  drawRankBars('rankPlant', Object.entries(plantSums).map(([k,v])=>({key:k,label:k,value:v})), {topN:5});
}
